...
#Check for the case of a single node (A singloe node doesn't even have a path to itself, else if will be conseided a path)
for i in range(verts):
 if degrees[k]==0:
...
while visited_nodes != [1]*verts:
  for i in range(verts):
      #First looks for unvisited nodes with defree 1 as they constitute the beginning of a path
